#!/usr/bin/env python3
"""
Test script     # Hover power calculation (Glauert formula)
    ...
    total_hover_power_ideal = hover_induced_power_per_rotor * rotor_count
    ...
    propeller_efficiency = 0.55  # Propeller efficiency at hover
    esc_efficiency = 0.95        # ESC efficiency
    system_efficiency = motor_efficiency * propeller_efficiency * esc_efficiency  # ~63%
    ...
    print()ting Glauert momentum theory implementation
...
def test_glauert_formula():
    ...
    for motor_config in ['single', 'coaxial']:
        ...
        # Test parameters - typical 10kg hexacopter
        mass = 10.0  # kg
        ...
        # Adjust rotor count based on configuration
        rotor_count = base_rotor_count * 2 if motor_config == 'coaxial' else base_rotor_count
        
        print(f"Test Configuration:")
        print(f"- Mass: {mass} kg")
        print(f"- Rotor diameter: {rotor_diameter} m")
        print(f"- Base rotor count: {base_rotor_count} (frame: hexa)")
        print(f"- Actual rotor count: {rotor_count} ({motor_config})")
    ...
    # Apply coaxial efficiency penalty if needed
    coaxial_efficiency = 0.87 if motor_config == 'coaxial' else 1.0
    ...
    if motor_config == 'coaxial':
        print(f"- Coaxial efficiency penalty: {coaxial_efficiency:.1%}")
    print()
    ...
    propeller_efficiency = 0.55  # Propeller efficiency at hover
    esc_efficiency = 0.95        # ESC efficiency
    system_efficiency = motor_efficiency * propeller_efficiency * esc_efficiency  # ~44.4%
    ...
    print()
        ...
        for speed in test_speeds:
            if speed == 0:
                ...
                profile_drag_coeff = 0.022  # Even higher drag coefficient for weaker sweet spot
                solidity_ratio = 0.12  # Higher solidity
                
                # Profile power increases with forward speed (additional drag on advancing blade)
                speed_factor = 1 + (speed / 7)**3  # Even steeper increase with forward speed
                ...
                # Parasitic drag power (vehicle body drag) - reduced for more realistic sweet spot
                parasitic_drag_coeff = 0.25  # Lower drag coefficient for more realistic sweet spot
